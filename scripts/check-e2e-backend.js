  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        ...
        console.error('');
        console.error('Seed the clothing tenant in django-crm first.')
        console.error('');
        process.exit(1);
      }
    ...
    const productsRes = await fetch(`${apiUrl}/v1/public/clothing/products/?page_size=1`)
    if (!productsRes.ok) {
      console.error('');
    ...
    if (!Array.isArray(products) || products.length === 0) {
      console.error('');
      console.error('E2E backend not ready: no products for clothing.')
      console.error('');
      console.error('Seed the clothing tenant via the django-crm admin or fixtures.')
      console.error('');
      process.exit(1);
    }
